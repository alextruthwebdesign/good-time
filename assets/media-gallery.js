    ...
    class MediaGallery extends HTMLElement {
      ...
      setActiveThumbnail(thumbnail) {
        ...
        const getActiveThumbnail = [
          ...this.elements.viewer.querySelectorAll("ul li"),
        ]?.findIndex((el) => el?.classList?.contains("is-active"));
        if (getActiveThumbnail >= 0) {
          const dots = this.elements.viewer?.querySelectorAll(".dots .dot");
          const activeDot = dots?.[getActiveThumbnail];
          if (activeDot) {
            dots?.forEach((dot) => {
